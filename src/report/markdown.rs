...
impl MarkdownReporter {
    ...
    fn generate_markdown(&self, snapshot: &Snapshot, findings: &[Finding]) -> ReportResult<String> {
        ...
        md.push_str(&format!("Generated by **KCPilot** v{}\n\n", snapshot.metadata.tool_version));
        ...
        if total == 0 {
            md.push_str("✅ **No issues found!** Your Kafka cluster appears to be healthy.\n\n");
        } else {
            md.push_str(&format!("⚠️ **{} issue(s) detected** in your Kafka cluster requiring attention.\n\n", total));
            
            if critical > 0 || high > 0 {
                md.push_str(&format!("🚨 **Immediate action required**: {} critical and {} high severity issues found.\n\n", critical, high));
            }
        }
        ...
        md.push_str(&format!("[{}{}] {:.0}%\n", "█".repeat(filled), "░".repeat(bar_length - filled), health_score));
        md.push_str("```\n\n");
        ...
        // Footer
        md.push_str("---\n\n");
        md.push_str("*This report was automatically generated by [KCPilot](https://softwaremill.github.io/kcpilot/)*\n");
    ...
    fn write_finding(&self, md: &mut String, num: usize, finding: &Finding) -> ReportResult<()> {
        ...
        if !finding.remediation.steps.is_empty() {
            ...
            md.push_str(&format!("- **Requires Downtime**: {}\n", 
                              if finding.remediation.requires_downtime { "Yes ⚠️" } else { "No ✅" }));
